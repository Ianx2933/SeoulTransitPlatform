"""
Resolve missing bus-stop coordinates using curated OD stop metadata first.

Priority:
1. curated.standard_node_id = node_info.노드ID.
2. curated.ars_id + curated.stop_name = node_info.정류장번호 + node_info.노드명.
...
def load_node_info() -> pd.DataFrame:
    ...
    # Normalize stop numbers / ARS values to five-character strings.
    # Left-pad four-digit numeric values with zero, e.g. 1234 -> 01234.
...
def attach_coordinates(ref: pd.DataFrame, node: pd.DataFrame) -> pd.DataFrame:
    """Attach coordinates to curated stop references using ordered matching rules."""
    ref = ref.copy()
    ref["curated_stop_name_norm"] = norm_name(ref["stop_name"])

    # 1) Match by exact standard node ID.
    exact = ref.merge(
        node,
        left_on="standard_node_id",
        right_on="canonical_node_id",
        how="left",
        suffixes=("", "_node"),
    )
    exact["match_method"] = "curated_standard_node_id_to_node_info_node_id"
    exact["match_confidence"] = 1.00

    matched = exact[exact["canonical_node_id"].notna()].copy()
    not_exact = exact[exact["canonical_node_id"].isna()][ref.columns].copy()

    # 2) Match by ARS and normalized stop name.
    by_name = not_exact.merge(
        node,
    ...
    by_name["match_confidence"] = 0.90

    matched_name = by_name[by_name["canonical_node_id"].notna()].copy()
    not_name = by_name[by_name["canonical_node_id"].isna()][ref.columns].copy()

    # 3) Fall back only when ARS is unique in node_info.
    ars_counts = node.groupby("node_info_ars_id", dropna=False)["canonical_node_id"].nunique().reset_index(name="node_info_ars_candidate_count")
    unique_ars = node.merge(ars_counts, on="node_info_ars_id", how="left")
    unique_ars = unique_ars[unique_ars["node_info_ars_candidate_count"] == 1].copy()
    by_ars = not_name.merge(
        unique_ars,
        left_on="ars_id",
        right_on="node_info_ars_id",
        how="left",
    )
    by_ars["match_method"] = "unique_node_info_ars_fallback"
    by_ars["match_confidence"] = 0.70

    matched_ars = by_ars[by_ars["canonical_node_id"].notna()].copy()
    unresolved = by_ars[by_ars["canonical_node_id"].isna()].copy()
    unresolved["unresolved_reason"] = "no_coordinate_candidate_after_curated_and_node_info_fallback"

    all_matched = pd.concat([matched, matched_name, matched_ars], ignore_index=True, sort=False)
...
def main() -> None:
    ...
    # demand node_id is an ARS-like stop code, so normalize it to five characters.
    # Non-numeric placeholders such as 0000~ are preserved and excluded below.
    ...
    require_columns(ref, {"route_name", "converted_route_id", "seq", "ars_id", "standard_node_id", "stop_name", "side", "reference_rows", "passenger_sum"}, "curated_reference")
    ...
    # Build an ARS-level fallback table only when demand_node_id maps to exactly one canonical node across all curated candidates.
    ...
    # Unresolved target IDs include IDs not found in resolved coordinate candidates plus coordinate-level unresolved rows.
    seen_ids = set(matched_candidates["ars_id"].dropna())
    ...
    unresolved_ids["unresolved_reason"] = unresolved_ids.get("unresolved_reason", "no_resolved_coordinate_candidate")
